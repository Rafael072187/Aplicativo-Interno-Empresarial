...
def criar_card(icone, numero, texto, coluna_card):
    ...
    coluna_esquerda, coluna_direta = container.columns([1, 2.5])
    coluna_esquerda.image(f"imagens/{icone}")
    coluna_direta.write(numero)
    coluna_direta.write(texto)
...
criar_card("total_orcado.png", f'{base_fechados["Valor Orçado"].sum():,}', "Total Orçado", coluna_esquerda)
criar_card("total_pago.png", f'{base_fechados["Valor Negociado"].sum():,}', "Total Pago", coluna_meio)
criar_card("desconto.png", f'{base_fechados["Desconto Concedido"].sum():,}', "Total Desconto", coluna_direita)
